...
void makeParticle(int x, int y)
{
	...
	p->s.center.x = x;
	p->s.center.y = y;
	p->velocity.y = ((float)rand()/(float)RAND_MAX)-.5;
	p->velocity.x = ((float)rand()/(float)RAND_MAX)-.5;
	++g.n;
}
}
}
...
void movement()
{
	...
	if(p->s.center.x<right&&p->s.center.x>left&&p->s.center.y<top&&p->s.center.y>bottom){
		if(p->s.center.y-top<5&&p->s.center.y-top>-5){
			p->velocity.y= (0-p->velocity.y)/1.5+rnd;
			p->s.center.y += .5;
		}
		...
		if(p->s.center.x-right<5&&p->s.center.x-right>-5){
			p->velocity.x=(0-p->velocity.x)/1.5+rnd;
			p->s.center.x += .5;
		}
		if(p->s.center.x-left<5&&p->s.center.x-left>-5){
			p->velocity.x=(0-p->velocity.x)/1.5+rnd;
			p->s.center.x -= .5;
		}
	...
	if (p->s.center.x<0||p->s.center.x>800) {
